...
#ggsave("figures/draft/Fig1.png", arrow_plot, width = 7, height = 5, units = "in", dpi = 600)
#ggsave("figures/draft/Fig1.pdf", arrow_plot, width = 6, height = 4, units = "in", dpi = 600)
```
...
N15.TL.fish.pred <- data_SIA_fish %>% 
  ...
  data_grid(TL_c = seq_range(TL_c, n = 100), Treatment) %>%
...
N15.TL.inv.pred <- data_SIA_inv %>% 
  ...
  group_by(Treatment, Site) %>%  
  data_grid(Length_c = seq_range(Length_c, n = 100), Treatment) %>%
...
#ggsave("figures/main/Fig2.png", fig2, width = 9, height = 6, units = "in", dpi = 600)
...
ggsave("figures/main/Fig3.png", fig3, width = 7, height = 5, units = "in", dpi = 600)
...
figs2 <- ggarrange(fish.fam.bio.plot, fish.fam.dens.plot + rremove("y.text"), widths = c(1.4, 1),
                             common.legend = T, nrow = 1)

figs2

#ggsave("figures/draft/FigS2.png", figs2, width = 7, height = 6, units = "in", dpi = 600)
...
figs3 <- ggarrange(fish.genspe.bio.plot, fish.genspe.dens.plot + rremove("y.text"), widths = c(1.4, 1),
                             common.legend = T, nrow = 1)

#ggsave("figures/supp/FigS3.png", figs3, width = 7, height = 7, units = "in", dpi = 600)
...
figs4 <- ggarrange(inv.red.fam.bio.plot, inv.red.fam.dens.plot + rremove("y.text"), 
                             common.legend = T, nrow = 1)

figs4

#ggsave("figures/supp/FigS4.png", figs4, width = 6, height = 7, units = "in", dpi = 600)
...
#ggsave("figures/main/Fig4.png", fig4, width = 6, height = 5, units = "in", dpi = 600)
...
p1 <- ggplot(data_SIA_fish_mm %>% 
  filter(GENSPE == "Chlidichthys chagosensis"), aes(x = Length, y = C13)) +
  geom_point() +
...
p2 <- ggplot(data_SIA_fish_mm %>% 
  filter(GENSPE == "Chlidichthys chagosensis"), aes(x = Length)) +
  geom_histogram() +
  geom_vline(xintercept = 30, col = "red", linetype = "dashed") +
  ylab("Density") +
  xlab("Length (mm)") +
...
p3 <- ggplot(data_SIA_fish_mm %>% 
  ...
  mutate(Group = case_when(
    Length < 30 ~ "< 30 mm",
    Length > 30 ~ "> 30 mm")), aes(x = Length, y = C13)) +
  geom_vline(xintercept = 30, col = "red", linetype = "dashed") +
...
chag_lt30 <- data_SIA_fish_mm %>% filter(GENSPE == "Chlidichthys chagosensis") %>% filter(Length < 30)  %>% transmute(Group = "C. chagosensis < 30 mm",  C13)
chag_ge30 <- data_SIA_fish_mm %>% filter(GENSPE == "Chlidichthys chagosensis") %>% filter(Length > 30) %>% transmute(Group = "C. chagosensis \u2265 30 mm", C13)
...
figs1 <- ggarrange(p2 + rremove("x.text") + rremove("xlab") + rremove("x.ticks"), 
                   p1 + rremove("x.text") + rremove("xlab") + rremove("x.ticks"), p3, p4, 
                   nrow = 3, align = "v", labels = c("a", "b", "c", "d"), heights = c(1,1,1.2, 1.2))

#ggsave("figures/supp/FigS1.png", figs1, width = 7, height = 5, units = "in", dpi = 600)
...
mix_Chlichag <- data_SIA_fish_mm %>% 
  filter(GENSPE == "Chlidichthys chagosensis" & Length > 30) %>% 
  select(C13, N15, Treatment, Length)
...
write.csv(mix_Chlichag, "output_data/mix_Chlichag.csv", row.names = FALSE)
write.csv(sources, "output_data/sources.csv", row.names = FALSE)

# Load the mixture/consumer data
Chlichag_mix <- load_mix_data("output_data/mix_Chlichag.csv", 
                            iso_names = c("C13","N15"), 
                            factors = c("Treatment"), 
                            fac_random = c(FALSE), 
                            fac_nested = c(FALSE), 
...
sources <- load_source_data("output_data/sources.csv",
...
plot.mix <- ggarrange(plot.mix.c + rremove("legend.title"), plot.mix.d + rremove("ylab") + rremove("legend.title"),
...
plot.mix
#ggsave("figures/draft/FigS4.png", plot.mix, width = 6, height = 5, units = "in", dpi = 600)
#ggsave("figures/draft/FigS4.pdf", plot.mix, width = 6, height = 5, units = "in", dpi = 600)
...
#ggsave("figures/draft/Fig6.png", uvc.plot, width = 6, height = 4, units = "in", dpi = 600)
#ggsave("figures/draft/Fig6.pdf", rda_plot, width = 6, height = 6, units = "in", dpi = 600)
