...
pisc_sum_bio<-rowSums(pisc_dat_bio_wide[,7:ncol(pisc_dat_bio_wide)])
...
uvc_benkwitt <- read.csv("data/UVC_priors.csv") %>% filter(Diet == "Invertivore" & Area > 100) %>% mutate(Species = str_replace_all(Species, "_", " ")) %>% 
  ...
  mutate(Weight = a*(Length^b),
         Weight_m = Weight/Area) %>% 
  select(-Coral_cover, -Abundance, -Structure)
...
inv_sum_bio<-rowSums(inv_dat_bio_wide[,7:ncol(inv_dat_bio_wide)])
...
# Sum biomass
pisc_sum_bio <- rowSums(pisc_dat_bio_wide[,6:ncol(pisc_dat_bio_wide)])
...
ggplot(uvc_pisc.prod.data, aes(x = Treatment, y = log(Prod))) +
  geom_boxplot()
...
priors_bio <- c( 
  prior(normal(-1.85, 0.70), class = "b", coef = "TreatmentRatty"),                  
...
inv_sum_bio<-rowSums(inv_dat_bio_wide[,6:ncol(inv_dat_bio_wide)])
...
priors_prod <- c( 
  prior(normal(0.48, 0.41), class = "b", coef = "TreatmentRatty"),                  
  prior(normal(3, 0.5), class = "Intercept"),                  
  prior(normal(0, 0.5), class = "sd"),                           
  prior(gamma(2, 0.1), class = "shape")
)
...
ggsave("figures/main/Fig5.jpg", uvc.plot, width = 6, height = 4, units = "in", dpi = 600)
ggsave("figures/main/Fig5.pdf", uvc.plot, width = 6, height = 4, units = "in", dpi = 600)
